...
struct
  ...
  let parse_remote_call (valenv, _, _) cgi_args =
    ...
    let i_fname = failwith "TODO resolve fname" (* int_of_string fname *) in
    let func, args =
      match fvs with
      | `Record [] when Lib.is_primitive_var i_fname ->
        `PrimitiveFunction (CommonTypes.Name.to_string (Lib.primitive_name i_fname), Some i_fname), args
      | `Record [] ->
