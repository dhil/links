...
sig
  ...
  val project : value sem * Label.t -> value sem
  ...
  val inject : Label.t * value sem * datatype -> value sem
  (* val case : *)
  ...
  val alien : var_info * Ident.Persistent.t * language * (var -> tail_computation sem) -> tail_computation sem

  val select : Label.t * value sem -> tail_computation sem
...
struct
  ...
  sig
    ...
    val alien_binding : var_info * Ident.Persistent.t * language -> var M.sem
