...
const LINKS = new (function() {
  ...
  function resolveMobileState(state, processes, handlers) {
    ...
    // register event handlers
    for (var i in handlers) {
      handlers[i].eventHandlers = eval(handlers[i].eventHandlers);
    ...
    while (true) {
        try {
            ...
            if (e instanceof Yield) {
                ...
                console.log("Yield count " + yieldCount);
                throw e;
            }
        }
    }
...
function resolveServerValueCPS(state, obj, k) {
    ...
    case "List":
        ...
        return _yield(function() { return resolveServerValueCPS(state, obj._head, function(head) {
            obj._head = head;
            return _yield(function () { return resolveServerValueCPS(state, obj._tail,
                                         function(tail) {
                                             obj._tail = tail;
                                             return applyK(k, obj);
                                         })
