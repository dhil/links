    ...
    kify : function kify(f) {
      return function () {
         // Nota bene: arguments is not a real array, hence no .pop()
          var kappa = arguments[arguments.length-2];
          //var effh = arguments[arguments.length-1];
          var args = Array.prototype.slice.call(arguments, 0, arguments.length-1); // FIXME : think about how handlers fit in here
...
function _wrapEventHandler(handler) {
  return function(event) {
    ...
    _handlingEvent = true;
      ...
      var _cont = function () { handler(event, _idy, _efferr ) }
...
// effect error: default "toplevel" effect handler
function _efferr(z,kappa,effh) { _error("Unhandled operation `" + z._label + "'.") }
...
function receive(c, kappa, effh) {
  ...
  if (_channels[c.channel][1 - c.direction].messages.length > 0) {
    ...
    _block_proc(current_pid,
                function () {
                  _current_pid = current_pid;
                    receive(c, kappa, effh);
                });
  }
}

// TODO: implement link
function link(c, d, kappa, effh) {
...
function _yieldCont(k, arg, effh) {
	++_yieldCount;
	if (_yieldCount == _yieldGranularity) {
		_yieldCount = 0;
		if (_handlingEvent) {
		    _theContinuation.v = function () { k(arg, effh) }; throw _theContinuation;
		} else {
			var current_pid = _current_pid;
		    setZeroTimeout(function() { _current_pid = current_pid; k(arg, effh) });
		}
	} else {
	    return k(arg, effh);
	}
