...
=======
      ...
      let p = processes[i];
      _spawnWithMessages(p.pid, p.process, p.messages);
>>>>>>> 619507dae1d53c53604d3ee829e8b42a65b1aeb6
    }
  ...
  function resolveMobileState(state, processes, handlers) {
    ...
    for (let i in handlers) {
<<<<<<< HEAD
      handlers[i].eventHandlers = eval(handlers[i].eventHandlers); // TODO(dhil): I think this redundant, since eventHandlers always ought to be a literal JSON object.
      handlers[i] = resolveServerValue(state, handlers[i]);
...
function resolveServerValueCPS(state, obj, k) {
    function applyK(k, arg) {
        ...
        if (_resolveDepth >= _resolveDepthMax) {
            ...
            throw new Yield(function() { return k(arg) });
        }
        return k(arg);
    }
    function _yield(f) {
    ...
    case "XML":
        return _yield(function() {
            return resolveServerValueCPS(state, obj._value, function(xml) {
                return _applyK(k, xml);
            });
        });
    case "Text":
        return _applyK(k, obj);
    case "NsNode":
        function __createChain(children, i, k) {
            return function() {
                return _yield(function() {
                    return resolveServerValueCPS(state, children[i], function(ith) {
                        children[i] = ith;
                        return applyK(k, children);
                    });
                });
            };
        }

        var chain = function(children) {
            obj.children = children;
            return applyK(k, obj);
        };

        for (let i in obj.messages) {
            chain = __createChain(obj.children, i, chain);
        }

        return chain(obj.children);
    case "Char":
    case "ClientDomRef":
    ...
    case "Record":
        var chain = function (dummy) { return applyK(k, obj) };
   ...
   function resolveServerValue2(state, obj) {
        if (obj._tag == null && obj.key != null) {
            obj.key = _lookupMobileKey(state, obj.key)
            return obj;
        }
        ...
        case "List":
            ...
            return LINKEDLIST.Cons(resolveServerValue(state, obj._head), resolveServerValue(state, obj._tail));
        case "Record":
            delete obj._tag;
            for (let k in obj)
        ...
        case "FunctionPtr":
            const f = (!TYPES.isObject(obj.environment)) ?
                eval(obj.func) :
                partialApply(eval(obj.func), eval(resolveServerValue(state, obj.environment)));
