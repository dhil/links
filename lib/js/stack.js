...
class GenericHandleFrame extends ContinuationFrame {
    ...
    apply(resumeValue) {
        return this.handle.call(null, this.f.toContinuation().reload, this.val, this.eff);
    }
}
GenericHandleFrame.prototype.handle = null;
GenericHandleFrame.prototype.comp = null;
GenericHandleFrame.prototype.val = null;
GenericHandleFrame.prototype.eff = null;

function _handle(f, arg, val, eff) {
    while(true) {
        try {
            return val(eff(f, arg));
        } catch (exn) {
            if (e instanceof ResumeError) {
                f = exn.f;
                continue;
            } else if (exn instanceof SaveContinuationError) {
                exn.extend(new GenericHandleFrame(_handle, exn, val, eff));
                throw exn;
            } else {
                throw exn;
            }
