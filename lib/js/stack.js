...
function _absurd(f) {
    try {
        ...
        if (e instanceof PerformOperationError) {
            ...
            throw "Save continuation error inside absurd";
        } else {
            throw e;
        }
    }
...
class FrameList {
    ...
    static splice(frames0, frames1) {
        if (frames0 == frames1) return frames0;
        let ptr = frames0;
...
class SaveContinuationError extends Error {
    ...
    extend(/* ContinuationFrame */ extension) { /* unit */
        this._new_frames.first.extend(extension);
        return;
    }

    instantiateTrapPoint(handler) {
        const h = this._new_frames.first;
        this._new_frames = new FrameList(h.instantiate(handler), this._new_frames.rest);
        return this;
    }

    setAbstractTrapPoint() {
        this._new_frames = new FrameList(new AbstractHandleFrame(), this._new_frames);
        return this;
    }
    ...
    toContinuation() { /* Continuation */
        return new LazyContinuation(this._new_frames, this._old_frames);
    }

    constructor(...args) {
        super(...args);
        this._new_frames = new FrameList(new AbstractHandleFrame(), null);
    }
}
        ...
        constructor(/* FrameList */ newFrames, /* FrameList */ oldFrames) {
            ...
            */;
            ...
            while (new_frames !== null) {
                //_print(JSON.stringify(new_frames))
...
/**
 * Lazily assembles the continuation upon reload.
 */
class LazyContinuation {

    constructor(new_frames, old_frames) {
        this._new_frames = new_frames;
        this._old_frames = old_frames;
        this._continuation = null;
    }

    reload(resumeValue) {
        if (this._continuation === null)
            this._continuation = new Continuation(this._new_frames, this._old_frames);
        return this._continuation.reload(resumeValue);
    }
}
...
class AbstractHandleFrame extends ContinuationFrame {
    get frames() { return this._frames; }
    set frames(fs) { this._frames = fs; }
    ...
    extend(extension) {
        this._frames = new FrameList(extension, this._frames);
        return;
    }

    apply(resumeValue) {
        throw "cannot resume under an abstract handler.";
    }

    instantiate(handler) {
        //_print(JSON.stringify(this._frames));
        return new GenericHandleFrame(handler, new LazyContinuation(this._frames, null /* invariant: an abstract handler never has any "old frames". */));
    }
}

AbstractHandleFrame.prototype._frames = null;
...
const _Resumption = (function() {
    return {
        "makeDeep": function(/* PerformOperationError */ exn, /* HandleFrame */ handle) {
            exn.instantiateTrapPoint(handle);
            const resume = exn.toContinuation();
            exn.setAbstractTrapPoint();
            return function(x) { return resume.reload(x); };
        }
    };
})();
