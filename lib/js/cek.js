...
/* Alien interface */
const _global = this; // pointer to the global namespace
const Alien = (function() {
    return {
        'apply': function(context, env, fname, args) {
            if (context[fname] !== undefined) {
                const fnptr = context[fname];
                return fnptr(...args, env);
            } else
                throw ("Undefined alien function " + fname);
        }
    };
}());
...
const IR = (function() {
    ...
    return {
        ...
        'Make': {
            ...
            'handle': function(computation, retClause, opClauses, depth, params = []) {
                return {'tag': HANDLE, 'comp': computation, 'return': retClause, 'operations': opClauses, 'depth': depth, 'params': params};
            },
            'resumption': function(depth, handler, frames, stack) {
                if (depth === DEEP)
                    return {'tag': RESUMPTION, 'depth': depth, 'owner': {'handler': handler, 'frames': frames}, 'kappa': stack};
                else
                    return {'tag': RESUMPTION, 'depth': depth, 'frames': frames, 'kappa': stack};
            },
            ...
            'list': function(list) {
                return {'tag': LIST, '_list': list};
            },
            'ref': function(v) {
                return {'tag': REF, '_contents': v};
            },
            'alien': function(jsname) {
                return {'tag': ALIEN, 'name': jsname}
            }
        },
        'match': function(ir) {
            return ir.tag;
...
const K = (function() {
    ...
    const Handler = (function() {
        ...
        const identity = (function() {
            ...
            return {'tag': IDENTITY, 'env': Env.empty, 'return': IR.Make.clause(x, IR.Make.computation([], IR.Make.ret(IR.Make.variable(x)))), 'clauses': {}, 'depth': IR.DEEP, 'params': []};
        }());

        return {
            'IDENTITY': IDENTITY,
            'USER_DEFINED': USER_DEFINED,
            'make': function(env, retClause, opClauses, depth, params = []) {
                let parameters = [];
                for (let i = 0; i < params.length; i++)
                    parameters[i] = params[i].binder;
                return {'tag': USER_DEFINED, 'env': env, 'return': retClause, 'clauses': opClauses, 'depth': depth, 'params': parameters};
            },
            'handle': function(kappa, op) {
                return; /* TODO */
            },
            'updateEnv': function(handler, env) {
                return {'tag': handler.tag, 'env': env, 'return': handler['return'], 'clauses': handler.clauses, 'depth': handler.depth, 'params': handler.params};
            },
            'identity': identity
        };
    }());
    ...
    return {
        ...
        'resume': function(kappa, resumption, args) {
            if (resumption.depth === IR.DEEP) {
                const nargs = args.length;
                const o = resumption.owner;
                let kappa0 = undefined;
                if (nargs > 1) {
                    let henv = o.handler.env;
                    for (let i = 1; i < nargs; i++) {
                        henv = Env.extend(henv, o.handler.params[i-1], args[i]);
                    }
                    const h = Handler.updateEnv(o.handler, henv);
                    kappa0 = List.rev(List.cons({'handler': h, 'frames': o.frames}, resumption.kappa));
                } else {
                    kappa0 = List.rev(List.cons(o, resumption.kappa));
                }
                return K.compose(kappa0, kappa);
            } else { // shallow
                const kappa0 = List.rev(resumption.kappa);
                const kappa1 = K.prependFrames(resumption.frames, kappa);
                return K.compose(kappa0, kappa1);
            }
        },
        'Handler': Handler,
...
const Primitive = (function() {
    ...
    function negate(env, args) {
        return IR.Make.constant(-args[0].value);
    }

    function list_eq(xs, ys) {
        let xs0 = xs._list;
        let ys0 = ys._list;
        let same = List.length(xs0) === List.length(ys0);

        while (same) {
            if (List.length(xs0) === 0 && List.length(ys0) === 0) {
                break;
            } else if (eq(List.hd(xs0), List.hd(ys0)) === false) {
                same = false;
                break;
            } else {
                xs0 = List.tl(xs0);
    ...
    return {
        'Function': {
            ...
            '%List.cons': function(env, args) {
                return IR.Make.list(List.cons(args[0], args[1]._list));
            },
            '%List.head': function(env, args) {
                const xs = args[0]._list;
                switch (List.match(xs)) {
                case List.NIL: throw "List.hd error.";
                case List.CONS: return List.hd(xs);
                }
            },
            '%List.tail': function(env, args) {
                const xs = args[0]._list;
                switch (List.match(xs)) {
                case List.NIL: throw "List.tl error.";
                case List.CONS: return IR.Make.list(List.tl(xs));
                }
            },
            '%List.concat': function(env, args) {
                const xs = args[0]._list;
                const ys = args[1]._list;
                return IR.Make.list(List.append(xs, ys));
            },
            '%List.length': function(env, args) {
                const xs = args[0]._list;
                return IR.Make.constant(List.length(xs));
            },
            /* References */
        ...
        'Constant': {
            '%List.nil': IR.Make.list(List.nil)
        }
    };
}());
    ...
    function toString(val) {
        ...
        case IR.LIST: {
            ...
            case List.NIL:
                return "[]";
            case List.CONS:
                let ys = toString(List.hd(xs));
                let zs = List.tl(xs);
                while (List.length(zs) > 0) {
                    ys = ys + ", " + toString(List.hd(zs));
                    zs = List.tl(zs);
                }
                return "[" + ys + "]"
            }
...
const CEK = (function() {
    ...
    /* Entry function */
    function interpret(funs, initial_env, program) {
        // Interpreter
        let mode = MODE.COMPUTATION;

        let control = program;         // C
        let environment = initial_env; // E
        ...
        while (true) {
            ...
            case MODE.TAIL_COMPUTATION: {
                ...
                case IR.APPLY: {
                    ...
                    case IR.ALIEN: { // alien javascript function
                        // this has the potential to obliterate
                        // everything, including existence itself.
                        const v = Alien.apply(_global, environment, fn.name, args);
                    ...
                    case IR.RESUMPTION: { // Delimited continuation
                        ...
                        kontinuation = K.resume(kappa, fn, args);
                        continue;
                        break;
                    }
                ...
                case IR.HANDLE: {
                    ...
                    if (handle.depth === IR.DEEP && handle.params.length > 0) {
                        const nparams = handle.params.length;
                        for (let i = 0; i < nparams; i++)
                            henv = Env.extend(henv, handle.params[i].binder, value(environment, handle.params[i].initial_value));
                    }
                    const handler = K.Handler.make(henv, handle['return'], handle.operations, handle.depth, handle.params);
